...
import edu.emory.mathcs.utils.ConcurrencyUtils.numberOfThreads
import edu.emory.mathcs.utils.ConcurrencyUtils.threadsBeginN_1D_FFT_2Threads
import edu.emory.mathcs.utils.ConcurrencyUtils.threadsBeginN_1D_FFT_4Threads
 ...
 * size of data
 */
class FloatFFT_1D(private val n: Int) {

    private val nBluestein = ConcurrencyUtils.nextPow2(n * 2 - 1)

    private val ip: IntArray = initip(n)

    private val w: FloatArray = initw(n)

    private var nw: Int = 0

    private var nc: Int = 0

    private val wtable: FloatArray = FloatArray(4 * n + 15)

    private val wtable_r: FloatArray = FloatArray(2 * n + 15)

    private val bk1: FloatArray = FloatArray(2 * nBluestein)

    private val bk2: FloatArray = FloatArray(2 * nBluestein)

    private val plan: Plans = initplan(n)
...
//                val ceil = Math.ceil(2 + (1 shl (Math.log(nBluestein + 0.5) / Math.log(2.0)).toInt() / 2))
//                this.ip = IntArray(2 + (2 + (1 shl (Math.log(nBluestein + 0.5) / Math.log(2.0)).toInt() / 2)))
...
//            val ceil = Math.ceil(2 + (1 shl (Math.log(n + 0.5) / Math.log(2.0)).toInt() / 2)).toInt()
//            val ceil = (2 + (1 shl (Math.log(n + 0.5) / Math.log(2.0)).toInt() / 2))
//            this.ip = IntArray(2 + ceil)
//            this.w = FloatArray(n)
            val twon = 2 * n
            nw = ip[0]
            if (twon > nw shl 2) {
...
//    @JvmOverloads
    fun complexForward(a: FloatArray, offa: Int = 0) {
        if (n == 1)
            return
        when (plan) {
            FloatFFT_1D.Plans.SPLIT_RADIX -> cftbsub(2 * n, a, offa, ip, nw, w)
            FloatFFT_1D.Plans.MIXED_RADIX -> cfftf(a, offa, -1)
            FloatFFT_1D.Plans.BLUESTEIN -> bluestein_complex(a, offa, -1)
        }
    }
    ...
    fun complexInverse(a: FloatArray, offa: Int, scale: Boolean) {
        if (n == 1)
            return
        when (plan) {
            FloatFFT_1D.Plans.SPLIT_RADIX -> cftfsub(2 * n, a, offa, ip, nw, w)
            FloatFFT_1D.Plans.MIXED_RADIX -> cfftf(a, offa, +1)
            FloatFFT_1D.Plans.BLUESTEIN -> bluestein_complex(a, offa, 1)
        }
        if (scale) {
...
//    @JvmOverloads
    fun realForward(a: FloatArray, offa: Int = 0) {
        ...
        when (plan) {
            ...
            FloatFFT_1D.Plans.BLUESTEIN -> bluestein_real_forward(a, offa)
        }
    }
...
//    @JvmOverloads
    fun realForwardFull(a: FloatArray, offa: Int = 0) {
        ...
        when (plan) {
            FloatFFT_1D.Plans.SPLIT_RADIX -> {
                realForward(a, offa)
                val nthreads = ConcurrencyUtils.numberOfThreads
                if (nthreads > 1 && n / 2 > ConcurrencyUtils.threadsBeginN_1D_FFT_2Threads) {
                    val k = n / 2 / nthreads
                    val futures = Array<Future<*>>(nthreads) { i ->
                        val firstIdx = i * k
                        val lastIdx = if (i == nthreads - 1) n / 2 else firstIdx + k
                        ConcurrencyUtils.submit(object : Runnable {
                            override fun run() {
                                var idx1: Int
                                var idx2: Int
            ...
            FloatFFT_1D.Plans.BLUESTEIN -> bluestein_real_full(a, offa, -1)
        }
    }
    ...
    fun realInverseFull(a: FloatArray, offa: Int, scale: Boolean) {
        ...
        when (plan) {
            FloatFFT_1D.Plans.SPLIT_RADIX -> {
                ...
                val nthreads = numberOfThreads
                if (nthreads > 1 && n / 2 > threadsBeginN_1D_FFT_2Threads) {
                    val k = n / 2 / nthreads
                    val futures = Array<Future<*>>(nthreads) { i ->
                        val firstIdx = i * k
                        val lastIdx = if (i == nthreads - 1) n / 2 else firstIdx + k
    ...
    internal fun cffti(n: Int, offw: Int) {
        if (n == 1)
            return
    ...
    private fun makewt(nw: Int) {
        ...
        if (nw > 2) {
            ...
            if (nwh == 4) {
                ...
                makeipt(nw)
                w[2] = (0.5f / Math.cos(delta2))
                w[3] = (0.5f / Math.cos(delta * 6))
                j = 4
                while (j < nwh) {
                    deltaj = delta * j
                    deltaj3 = 3 * deltaj
    ...
    private fun makect(nc: Int, c: FloatArray, startc: Int) {
        ...
        if (nc > 1) {
            ...
            while (j < nch) {
                deltaj = delta * j
                c[startc + j] = (0.5f * Math.cos(deltaj))
                c[startc + nc - j] = (0.5f * Math.sin(deltaj))
                j++
            }
        }
    }
    ...
    private fun bluestein_complex(a: FloatArray, offa: Int, isign: Int) {
        ...
        var nthreads = 1
        val threads = numberOfThreads
        if (threads > 1 && n > threadsBeginN_1D_FFT_2Threads) {
            nthreads = 2
            if (threads >= 4 && n > threadsBeginN_1D_FFT_4Threads) {
                nthreads = 4
            }
            var k = n / nthreads
            val futures = Array<Future<*>>(nthreads) { i ->
            ...
            for (i in 0 until nthreads) {
                ...
                futures[i] = ConcurrencyUtils.submit(object : Runnable {
                    override fun run() {
                        if (isign > 0) {
                            for (i in firstIdx until lastIdx) {
                                val idx1 = 2 * i
    ...
    private fun bluestein_real_inverse2(a: FloatArray, offa: Int) {
        ...
        if (threads > 1 && n > threadsBeginN_1D_FFT_2Threads) {
            ...
            k = nBluestein / nthreads
            val futures2 = Array<Future<*>>(nthreads) { i ->
                val firstIdx = i * k
                val lastIdx = if (i == nthreads - 1) nBluestein else firstIdx + k
                ConcurrencyUtils.submit(object : Runnable {
                    override fun run() {
            ...
            ConcurrencyUtils.waitForCompletion(futures2)
    ...
    /*---------------------------------------------------------
      ...
      --------------------------------------------------------*/
    internal fun rfftf(a: FloatArray, offa: Int) {
        if (n == 1)
            return
        var l1: Int
        var l2: Int
    ...
    /*---------------------------------------------------------
      ...
      --------------------------------------------------------*/
    internal fun rfftb(a: FloatArray, offa: Int) {
        if (n == 1)
            return
        var l1: Int
        var l2: Int
    ...
    /*-------------------------------------------------
      ...
      -------------------------------------------------*/
    internal fun radf2(ido: Int, l1: Int, `in`: FloatArray, in_off: Int, out: FloatArray, out_off: Int, offset: Int) {
        var i: Int
        var ic: Int
        val idx0: Int
        var idx1: Int
    ...
    /*-------------------------------------------------
      ...
      -------------------------------------------------*/
    internal fun radb2(ido: Int, l1: Int, `in`: FloatArray, in_off: Int, out: FloatArray, out_off: Int, offset: Int) {
        var i: Int
        var ic: Int
        var t1i: Float
        var t1r: Float
    ...
    /*-------------------------------------------------
      ...
      -------------------------------------------------*/
    internal fun radf3(ido: Int, l1: Int, `in`: FloatArray, in_off: Int, out: FloatArray, out_off: Int, offset: Int) {
        val taur = -0.5f
    ...
    /*-------------------------------------------------
      ...
      -------------------------------------------------*/
    internal fun radb3(ido: Int, l1: Int, `in`: FloatArray, in_off: Int, out: FloatArray, out_off: Int, offset: Int) {
        val taur = -0.5f
        val taui = 0.866025403784438707610604524234076962f
        var i: Int
        var ic: Int
    ...
    /*-------------------------------------------------
      ...
      -------------------------------------------------*/
    internal fun radf4(ido: Int, l1: Int, `in`: FloatArray, in_off: Int, out: FloatArray, out_off: Int, offset: Int) {
    ...
    /*-------------------------------------------------
      ...
      -------------------------------------------------*/
    internal fun radb4(ido: Int, l1: Int, `in`: FloatArray, in_off: Int, out: FloatArray, out_off: Int, offset: Int) {
    ...
    /*-------------------------------------------------
      ...
      -------------------------------------------------*/
    internal fun radf5(ido: Int, l1: Int, `in`: FloatArray, in_off: Int, out: FloatArray, out_off: Int, offset: Int) {
    ...
    /*-------------------------------------------------
      ...
      -------------------------------------------------*/
    internal fun radb5(ido: Int, l1: Int, `in`: FloatArray, in_off: Int, out: FloatArray, out_off: Int, offset: Int) {
        val tr11 = 0.309016994374947451262869435595348477f
        val ti11 = 0.951056516295153531181938433292089030f
        val tr12 = -0.809016994374947340240566973079694435f
        val ti12 = 0.587785252292473248125759255344746634f
    ...
    /*---------------------------------------------------------
      ...
      --------------------------------------------------------*/
    internal fun radfg(ido: Int, ip: Int, l1: Int, idl1: Int, `in`: FloatArray, in_off: Int, out: FloatArray, out_off: Int, offset: Int) {
        var idij: Int
        val ipph: Int
        var j2: Int
        var ic: Int
    ...
    /*---------------------------------------------------------
      ...
      --------------------------------------------------------*/
    internal fun radbg(ido: Int, ip: Int, l1: Int, idl1: Int, `in`: FloatArray, in_off: Int, out: FloatArray, out_off: Int, offset: Int) {
        var idij: Int
        val ipph: Int
        var j2: Int
        var ic: Int
    ...
    /*---------------------------------------------------------
      ...
      --------------------------------------------------------*/
    internal fun cfftf(a: FloatArray, offa: Int, isign: Int) {
        var idot: Int
        var l1: Int
        var l2: Int
        var na: Int
    ...
    internal fun passf2(ido: Int, l1: Int, `in`: FloatArray, in_off: Int, out: FloatArray, out_off: Int, offset: Int, isign: Int) {
        var t1i: Float
        var t1r: Float
        val iw1: Int
        iw1 = offset
    ...
    /*----------------------------------------------------------------------
       ...
       isign is +1 for backward and -1 for forward transforms
      ----------------------------------------------------------------------*/
    internal fun passf3(ido: Int, l1: Int, `in`: FloatArray, in_off: Int, out: FloatArray, out_off: Int, offset: Int, isign: Int) {
        val taur = -0.5f
        val taui = 0.866025403784438707610604524234076962f
        var ci2: Float
        var ci3: Float
        ...
        if (ido == 2) {
            for (k in 1..l1) {
                ...
                ti2 = i1i + i2i
                ci2 = i3i + taur * ti2
                cr3 = isign.toFloat() * taui * (i1r - i2r)
                ci3 = isign.toFloat() * taui * (i1i - i2i)
            ...
            for (k in 1..l1) {
                ...
                while (i < ido - 1) {
                    ...
                    ti2 = a1i + a2i
                    ci2 = a3i + taur * ti2
                    cr3 = isign.toFloat() * taui * (a1r - a2r)
                    ci3 = isign.toFloat() * taui * (a1i - a2i)
                    dr2 = cr2 - ci3
                    dr3 = cr2 + ci3
                    di2 = ci2 + cr3
                    di3 = ci2 - cr3
    ...
    /*----------------------------------------------------------------------
       ...
       isign is +1 for backward and -1 for forward transforms
      ----------------------------------------------------------------------*/
    internal fun passf4(ido: Int, l1: Int, `in`: FloatArray, in_off: Int, out: FloatArray, out_off: Int, offset: Int, isign: Int) {
        var ci2: Float
        var ci3: Float
        var ci4: Float
        var cr2: Float
    ...
    /*----------------------------------------------------------------------
       ...
       isign is +1 for backward and -1 for forward transforms
      ----------------------------------------------------------------------*/
    internal fun passf5(ido: Int, l1: Int, `in`: FloatArray, in_off: Int, out: FloatArray, out_off: Int, offset: Int, isign: Int)
    ...
    /*----------------------------------------------------------------------
       ...
       isign is +1 for backward and -1 for forward transforms
      ----------------------------------------------------------------------*/
    internal fun passfg(nac: IntArray, ido: Int, ip: Int, l1: Int, idl1: Int, `in`: FloatArray, in_off: Int, out: FloatArray, out_off: Int, offset: Int, isign: Int) {
        var idij: Int
        var idlj: Int
        val idot: Int
        val ipph: Int
    ...
    private fun cftfsub(n: Int, a: FloatArray, offa: Int, ip: IntArray, nw: Int, w: FloatArray) {
        if (n > 8) {
            if (n > 32) {
                cftf1st(n, a, offa, w, nw - (n shr 2))
                if (numberOfThreads > 1 && n > threadsBeginN_1D_FFT_2Threads) {
    ...
    private fun cftbsub(n: Int, a: FloatArray, offa: Int, ip: IntArray, nw: Int, w: FloatArray) {
        if (n > 8) {
            if (n > 32) {
    ...
    private fun cftrec4_th(n: Int, a: FloatArray, offa: Int, nw: Int, w: FloatArray) {
        ...
        idiv4 = 0
        m = n shr 1
        if (n > threadsBeginN_1D_FFT_4Threads) {
            nthreads = 4
            idiv4 = 1
            m = m shr 1
        }
        ...
        while (i < nthreads) {
            ...
            if (i != idiv4) {
                futures[idx++] = ConcurrencyUtils.submit(object : Runnable {
                    override fun run() {
                        var isplt: Int
                        var j: Int
                        var k: Int
        ...
        ConcurrencyUtils.waitForCompletion(futures.filterNotNull().toTypedArray())
    }
    ...
    private fun scale(m: Float, a: FloatArray, offa: Int, complex: Boolean) {
        ...
        val nthreads = numberOfThreads
        if (nthreads > 1 && n2 >= threadsBeginN_1D_FFT_2Threads) {
            val k = n2 / nthreads
            val futures = Array<Future<*>>(nthreads) { i ->
    ...
    companion object {
        ...
        private val PI = 3.14159265358979311599796346854418516f

        private val TWO_PI = 6.28318530717958623199592693708837032f
        ...
        private fun initplan(n: Int): Plans {
            return if (!ConcurrencyUtils.isPowerOf2(n)) {
                if (getReminder(n, factors) >= 211) {
                    Plans.BLUESTEIN
                } else {
                    Plans.MIXED_RADIX
        ...
        private fun initw(n: Int): FloatArray {
            if (n < 1) {
                throw IllegalArgumentException("n must be greater than 0")
            }
            ...
            if (!ConcurrencyUtils.isPowerOf2(n)) {
                if (getReminder(n, factors) >= 211) {
                    val nBluestein = ConcurrencyUtils.nextPow2(n * 2 - 1)

                    return FloatArray(nBluestein)
                }
            } else {
                return FloatArray(n)
            }
        ...
        private fun initip(n: Int): IntArray {
            if (n < 1) {
                throw IllegalArgumentException("n must be greater than 0")
            }

            if (!ConcurrencyUtils.isPowerOf2(n)) {
                if (FloatFFT_1D.getReminder(n, FloatFFT_1D.factors) >= 211) {
                    val nBluestein = ConcurrencyUtils.nextPow2(n * 2 - 1)
                    return IntArray(2 + (2 + (1 shl (Math.log(nBluestein + 0.5) / Math.log(2.0)).toInt() / 2)))
                }
            } else {
                return IntArray(2 + (2 + (1 shl (Math.log(n + 0.5) / Math.log(2.0)).toInt() / 2)))
            }
            return IntArray(0)
        }
    }
